...
func NewInitializedServer(configuration config.Config) (*gin.Engine, *sql.DB) {
	...
	productService := cache.NewProductCacheService(&productRepository, &aprioriRepository, redisLibrary, storageLibrary, db)
	transactionService := service.NewTransactionService(&transactionRepository, &productRepository, db)
	aprioriService := service.NewAprioriService(&transactionRepository, &productRepository, &aprioriRepository, storageLibrary, db)
