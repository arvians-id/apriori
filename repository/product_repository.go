...
func (repository *productRepository) Create(ctx context.Context, tx *sql.Tx, product entity.Product) (entity.Product, error) {
	...
	execContext, err := tx.ExecContext(ctx, query, product.Code, product.Name, product.Description, product.CreatedAt, product.UpdatedAt)
	if err != nil {
		return entity.Product{}, err
	}
...
func (repository *productRepository) Delete(ctx context.Context, tx *sql.Tx, product entity.Product) error {
	query := "DELETE FROM products WHERE code = ?"
	_, err := tx.ExecContext(ctx, query, product.Code)
	if err != nil {
		return err
	}
